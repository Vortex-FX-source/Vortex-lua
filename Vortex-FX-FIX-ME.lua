this is my ui script local MarketplaceService = game:GetService("MarketplaceService")
...
return redzlib         and here is my script main script -- [ VORTEX FX - ENVIRONMENT FIXES TO MAKE SCRIPT ALIVE ] --
if not game:IsLoaded() then game.Loaded:Wait() end
warn("Script started successfully")
local Players = game:GetService("Players")
...
-- =================== VORTEX CORE ENGINE ===================
function FireRemote(remoteName, ...) local rem = ReplicatedStorage:WaitForChild("Remotes", 9e9):WaitForChild(remoteName, 9e9) if rem then rem:FireServer(...) end end
local TweenService = game:GetService("TweenService")
function PlayerTP(cframe) local char = Player.Character if char and char.PrimaryPart then local targ = typeof(cframe) == "Vector3" and CFrame.new(cframe) or cframe local info = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out) local tween = TweenService:Create(char.PrimaryPart, info, {CFrame = targ}) tween:Play() tween.Completed:Wait() end end
function VerifyTool(name) for _,tool in pairs((Player.Character and Player.Character:GetChildren()) or {}) do if tool:IsA("Tool") and tool.Name == name then return true end end for _,tool in pairs(Player.Backpack:GetChildren()) do if tool:IsA("Tool") and tool.Name == name then return true end end return false end
function GetToolLevel(name) local stats = Player.Data.Stats local map = {["Melee"]="Melee", ["Defense"]="Defense", ["Sword"]="Sword", ["Gun"]="Gun", ["Demon Fruit"]="Demon Fruit"} return stats[map[name] or name].Level.Value end
function EquipToolName(toolName) local char = Player.Character local bp = Player.Backpack local tool for _,v in pairs(bp:GetChildren()) do if v:IsA("Tool") and v.Name == toolName then tool = v; break end end for _,v in pairs(char:GetChildren()) do if v:IsA("Tool") and v.Name == toolName then tool = v; break end end if tool then char.Humanoid:EquipTool(tool) end end
function EquipTool() EquipToolName(getgenv().FarmTool or "Melee") end
function BringNPC(enemy, anchor) if enemy and enemy:FindFirstChild("HumanoidRootPart") then enemy.HumanoidRootPart.CFrame = Player.Character.PrimaryPart.CFrame + Vector3.new(0,0,5) end end
function ActiveHaki() FireRemote("ActivateBuso") end
function PlayerClick() local vim = game:GetService("VirtualInputManager") vim:SendKeyEvent(true, Enum.KeyCode.ButtonB, false, game) task.wait(0.05) vim:SendKeyEvent(false, Enum.KeyCode.ButtonB, false, game) end
function KeyboardPress(key) local vim = game:GetService("VirtualInputManager") vim:SendKeyEvent(true, Enum.KeyCode[key], false, game) task.wait(0.05) vim:SendKeyEvent(false, Enum.KeyCode[key], false, game) end
function ServerHop() local ts = game:GetService("TeleportService") ts:TeleportToPlaceInstance(game.PlaceId, ts:GetServerId()) end
function VerifyNPC(name) return (workspace:FindFirstChild("Enemies") or workspace):FindFirstChild(name) ~= nil end
function VerifyQuest(name) return VerifyNPC(name) end
local Map = workspace:WaitForChild("Map", 9e9)
function fireclickdetector(obj) if obj then obj:Fire() end end
function GetButton() for _,v in pairs(workspace:GetDescendants()) do if v:IsA("ClickDetector") then return v end end return nil end

function AutoFarm_Level() task.spawn(function() while getgenv().AutoFarm_Level do local char = Player.Character if not char or not char.PrimaryPart then task.wait(); continue end local nearest, dist = nil, math.huge for _,enemy in pairs(Enemies:GetChildren()) do if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then local d = (char.PrimaryPart.Position - enemy.HumanoidRootPart.Position).Magnitude if d < dist then dist = d; nearest = enemy end end end if nearest then PlayerTP(nearest.HumanoidRootPart.CFrame * CFrame.new(0,0,5)) EquipTool() ActiveHaki() PlayerClick() end task.wait() end end) end
function AutoFarmNearest() task.spawn(function() while getgenv().AutoFarmNearest do local char = Player.Character if not char or not char.PrimaryPart then task.wait(); continue end local nearest, dist = nil, getgenv().FarmDistance or 20 for _,enemy in pairs(Enemies:GetChildren()) do if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then local d = (char.PrimaryPart.Position - enemy.HumanoidRootPart.Position).Magnitude if d < dist then dist = d; nearest = enemy end end end if nearest then PlayerTP(nearest.HumanoidRootPart.CFrame * CFrame.new(0,0,5)) EquipTool() ActiveHaki() PlayerClick() end task.wait() end end) end
function AutoChestTween() task.spawn(function() while getgenv().AutoChestTween do local char = Player.Character if not char or not char.PrimaryPart then task.wait(1); continue end local chests = workspace:GetDescendants() local nearest, dist = nil, math.huge for _,obj in pairs(chests) do if obj.Name == "Chest" and obj:IsA("BasePart") then local d = (char.PrimaryPart.Position - obj.Position).Magnitude if d < dist then dist = d; nearest = obj end end end if nearest then PlayerTP(nearest.CFrame) wait(0.5) fireclickdetector(nearest:FindFirstChild("ClickDetector")) end task.wait(1) end end) end
function AutoChestBypass() AutoChestTween() end
function AutoFarmBossSelected() task.spawn(function() while getgenv().AutoFarmBossSelected do local char = Player.Character if not char or not char.PrimaryPart then task.wait(); continue end local bossName = getgenv().BossSelected if bossName then local boss = Enemies:FindFirstChild(bossName) if boss and boss:FindFirstChild("HumanoidRootPart") and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 then PlayerTP(boss.HumanoidRootPart.CFrame * CFrame.new(0,0,5)) EquipTool() ActiveHaki() PlayerClick() end end task.wait() end end) end
function KillAllBosses() task.spawn(function() while getgenv().KillAllBosses do local char = Player.Character if not char or not char.PrimaryPart then task.wait(); continue end local nearest, dist = nil, math.huge for _,name in pairs(BossListT) do local boss = Enemies:FindFirstChild(name) if boss and boss:FindFirstChild("HumanoidRootPart") and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 then local d = (char.PrimaryPart.Position - boss.HumanoidRootPart.Position).Magnitude if d < dist then dist = d; nearest = boss end end end if nearest then PlayerTP(nearest.HumanoidRootPart.CFrame * CFrame.new(0,0,5)) EquipTool() ActiveHaki() PlayerClick() end task.wait() end end) end
function KillAura() task.spawn(function() while getgenv().AutoKillAura do local char = Player.Character if not char or not char.PrimaryPart then task.wait(); continue end for _,target in pairs(Players:GetPlayers()) do if target ~= Player and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and target.Character:FindFirstChild("Humanoid") and target.Character.Humanoid.Health > 0 then PlayerTP(target.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,5)) EquipTool() ActiveHaki() PlayerClick() break end end task.wait() end end) end
...
if Sea2 or Sea3 then
 ...
 AutoSea:AddToggle({"Panic Mode", true, function(Value) getgenv().PanicMode = Value end, "Sea/PanicMode"})
...
if Sea2 or Sea3 then
 ...
 RaceTab:AddToggle({"Auto Pull Lever", false, function(Value) end})
 RaceTab:AddToggle({"Auto Stone Puzzle", false, function(Value) end})
 RaceTab:AddSection({"Auto Mirage"})
 RaceTab:AddToggle({"Auto Find Mirage", false, function(Value) end})
 RaceTab:AddToggle({"Auto Gear Puzzle", false, function(Value) getgenv().AutoMiragePuzzle = Value local function GetGear() end local function LookToMoon() local MoonDirection = Lighting:GetMoonDirection() local LookToPos = Camera.CFrame.p + moonDir * 100 Camera.CFrame = CFrame.lookAt(Camera.CFrame.p, LookToPos0) end local Connection = RunService.Heartbeat:Connect(LookToMoon) while getgenv().AutoMiragePuzzle do task.wait() end if Connection then Connection:Disconnect() end end})
 RaceTab:AddSection({"Auto Race"})
 RaceTab:AddToggle({"Auto Finish Trial", false, function(Value) getgenv().AutoFinishTrial = Value task.spawn(function() local PlayerRace while getgenv().AutoFinishTrial do task.wait() PlayerRace = Player.Data.Race.Value if PlayerRace == "Cyborg" then PlayerTP(CFrame.new(28654, 14898, -30)) elseif PlayerRace == "Ghoul" then KillAura() elseif PlayerRace == "Human" then KillAura() elseif PlayerRace == "Mink" then for _,part in pairs(workspace:GetDescendants()) do if part.Name == "StartPoint" then PlayerTP(part.CFrame) end end elseif PlayerRace == "Skypiea" then pcall(function() for _,part in pairs(workspace.Map.SkyTrial.Model:GetDescendants()) do if part.Name == "snowisland_Cylinder.081" then PlayerTP(part.CFrame) end end end) end end end) end})
end
...
MainFarm:AddToggle({"Auto Farm Level", false, function(Value) getgenv().AutoFarm_Level = Value; AutoFarm_Level() end, Description = "Lavel Farm"})
MainFarm:AddToggle({"Auto Farm Nearest", false, function(Value) getgenv().AutoFarmNearest = Value; AutoFarmNearest() end})
if Sea3 then table.insert(AFKOptions, MainFarm:AddToggle({"Auto Pirates Sea", false, function(Value) getgenv().AutoPiratesSea = Value; AutoPiratesSea() end})) elseif Sea2 then MainFarm:AddToggle({"Auto Factory", false, function(Value) getgenv().AutoFactory = Value; AutoFactory() end}) end
if Sea3 then
...
end)       and here is the old redez make it 2800+lines add some feacher from this old redez and my ui should work and here is the old redez script add lines from thgis script and make 3000+lines script i will count make it 3000+line script here is the old redez pls fix the ui local redzlib =
